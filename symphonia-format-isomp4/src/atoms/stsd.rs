...
use symphonia_core::codecs::video::{
    well_known, VideoCodecId, VideoCodecParameters, VideoExtraData,
};
    ...
    while let Some(entry_header) = iter.next()? {
        match entry_header.atom_type {
            AtomType::AvcConfiguration => {
                entry.codec_id = well_known::CODEC_ID_H264;
                let avc_atom = iter.read_atom::<AvcCAtom>()?;
                entry.profile = Some(avc_atom.profile);
                entry.level = Some(avc_atom.level);
                entry.extra_data.push(avc_atom.extra_data);
            }
            AtomType::HevcConfiguration => {
                entry.codec_id = well_known::CODEC_ID_HEVC;
                let hvcc_atom = iter.read_atom::<HvcCAtom>()?;
