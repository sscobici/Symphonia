...
pub fn build_mediainfo_command(path: &Path) -> Command {
...
pub fn get_mediainfo_format(opts: &InfoTestOptions) -> Result<Box<dyn FormatReader>> {
    ...
    let format_info = get_format_info(general, data.tracks.get(1));
        ...
        Some(format) => {
            let (format, short_name) = match format.as_str() {
                ...
                "MPEG Audio" => {
                    ...
                    let profile = first.and_then(|t| t.format_profile.as_ref());
                    ...
                    match profile {
                        ...
                        None => {
                            println!(
                                "mediainfo: first track doesn't have \"Format_Profile\" or first track is missing"
                            );
                            (FORMAT_ID_NULL, "Unknown")
                        }
                    }
