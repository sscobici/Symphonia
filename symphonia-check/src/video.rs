...
use crate::{get_codec_type, RefProcess, VideoTestDecoder, VideoTestOptions};
        ...
        loop {
            ...
            if line.starts_with("[PACKET]") {
                ...
                match key {
                    ...
                    "pts" => {
                        pts = value.parse::<u64>().unwrap_or_default();
                    }
                    "dts" => {
                        dts = value.parse::<i64>().unwrap_or_default();
                    }
                    "pos" => {
                        pos = value.parse::<u64>().unwrap_or_default();
                    }
                    "stream_index" => {
    ...
    for exp in packet_iterator {
        ...
        let different = match (exp.codec_type.as_str(), codec_param) {
            (AUDIO, Some(CodecParameters::Audio(_))) | (VIDEO, Some(CodecParameters::Video(_))) => {
                ...
                exp.pts != act.pts || exp.dts != act.dts as i64
            }
            _ => true,
        };
        ...
        if different {
            ...
            println!(
                ...
                exp.codec_type, exp.dts, exp.pts, exp.pos
            );
            println!(
